 ...
 */
export async function getRefundAnalytics(startDate?: string, endDate?: string) {
  let dateFilter: any = {};
  if (startDate || endDate) {
    dateFilter.refundedAt = {};
    if (startDate) dateFilter.refundedAt.gte = new Date(startDate);
    if (endDate) dateFilter.refundedAt.lte = new Date(endDate);
  }

  const [totalRefunds, totalRefundAmount, partialRefunds, fullRefunds] =
    await Promise.all([
 ...
 */
export async function validateRefundEligibility(paymentId: string): Promise<{
  eligible: boolean;
  reason?: string;
  ...
  if (payment.match && payment.match.scheduledAt < now) {
    ...
    return {
      eligible: true,
      maxRefundable: maxRefundable * 0.5, // 50% refund for completed matches
