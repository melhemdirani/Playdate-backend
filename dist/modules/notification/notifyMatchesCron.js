...
exports.closeExpiredRatingWindows = exports.notifyRatingDeadlines = exports.notifyUpcomingMatches = void 0;
const client_1 = require("@prisma/client");
const notificationService_1 = require("./notificationService");
const prisma = new client_1.PrismaClient();
// Notify users about matches starting soon (2 hours and 30 minutes before)
async function notifyUpcomingMatches(batchSize = 100) {
    const now = new Date();
            ...
            await Promise.all(batch.map((participant) => {
                ...
                return (0, notificationService_1.createNotification)({
                    ...
                    data: {
                        ...
                        match,
                        gameType: match.game.name,
                        location: (_a = match.location) === null || _a === void 0 ? void 0 : _a.name,
    ...
    for (const match of matchesInThirtyMinutes) {
        for (let i = 0; i < match.participants.length; i += batchSize) {
            const batch = match.participants.slice(i, i + batchSize);
            await Promise.all(batch.map((participant) => (0, notificationService_1.createNotification)({
                userId: participant.userId,
...
exports.notifyUpcomingMatches = notifyUpcomingMatches;
// Check for rating deadlines (23 hours after match completion - 1 hour warning)
async function notifyRatingDeadlines() {
    ...
    for (const match of completedMatches) {
        for (const participant of match.participants) {
            ...
            if (!hasRated) {
                ...
                const redirectLink = match.maxPlayers === 2
...
exports.notifyRatingDeadlines = notifyRatingDeadlines;
...
async function closeExpiredRatingWindows() {
    ...
    if (expiredMatches.length > 0) {
        console.log(`Found ${expiredMatches.length} matches with expired rating windows (completed >24h ago)`);
    }
}
exports.closeExpiredRatingWindows = closeExpiredRatingWindows;
