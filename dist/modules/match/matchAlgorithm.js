...
const getMatches = async (query) => {
    try {
        ...
        const enrichedMatches = await Promise.all(matches.map(async (match) => {
            ...
            const isTeamGame = ["basketball", "volleyball"].includes(gameName) ||
            ...
            const levelScore = getLevelScore(isTeamGame, userLevel, teamAverageLevel);
            ...
            const finalScore = fillScore + levelScore + locationScore + timeScore + rarityScore;
            return {
                ...match,
                score: finalScore,
                fillRate,
        ...
        const exactMatches = enrichedMatches
            ...
            .slice(0, 3);
        const restMatches = enrichedMatches.filter((m) => !exactMatches.includes(m));
...
function getGameRarityBoost(user, gameName) {
    const priority = {
        squash: 10,
        basketball: 8,
        volleyball: 6,
...
function averageTeamLevel(participants, gameId) {
    const levels = participants
        .map((p) => { var _a; return (_a = p.user.games.find((g) => g.gameId === gameId)) === null || _a === void 0 ? void 0 : _a.level; })
        .filter(Boolean)
...
function getLevelScore(isTeamGame, userLevel, avgLevel) {
...
function getLocationScore(matchLoc, userLoc) {
    if (!matchLoc || !userLoc)
        return 0;
    const dist = haversineDistance(matchLoc.latitude, matchLoc.longitude, userLoc.latitude, userLoc.longitude);
    return dist <= 3 ? 25 : dist <= 10 ? 15 : 0;
}
// Assigns score if match time aligns with user's preferred time slots
function getTimeScore(scheduledAt, preferred) {
